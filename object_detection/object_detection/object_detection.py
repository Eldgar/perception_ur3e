...
class ObjectDetection(Node):
    ...
    def callback(self, msg: PointCloud2) -> None:
        self.get_logger().info(f"Received PointCloud2 message with frame: {msg.header.frame_id}")
        try:
            # Convert PointCloud2 msg to pcl point cloud
            cloud = self.from_ros_msg(msg)

            # Filtered cloud for surface detection
            filtered_cloud_plane = self.filter_cloud(cloud, max_x_dist=0.8, min_height=-0.5, max_height=0.0)
            # Filtered cloud for object detection
            filtered_cloud_objects = self.filter_cloud(cloud, max_x_dist=0.8, min_height=0.0, max_height=0.4)
            ...
            bench_clusters, surface_centroids, surface_dimensions = self.extract_clusters(plane_cloud, "Table")
    ...
    def pub_object_detected(self, centroids: List[List[float]], dimensions: List[List[float]]) -> None:
        ...
        for idx, (centroid, dimension) in enumerate(zip(centroids, dimensions)):
            ...
            object_msg.position.x = centroid[0] + 0.0115
            object_msg.position.y = centroid[1] - 0.009
